# dashboard from Samuel Mathias on Medium: 
# https://python.plainenglish.io/creating-interactive-analytics-dashboards-using-pythons-streamlit-43a770bc3194
...
def load_data():
    ...
    # convert date colomn 'Order Date' ('dd/mm/yyyy hh:mm:ss') to datetime format
    df['Order Date'] = pd.to_datetime(df['Order Date'], format='%d/%m/%Y %H:%M:%S')

    # Extract the year and store it as a new column
    df['year'] = df['Order Date'].dt.year

    # convert date column 'Ship Date' ('dd/mm/yyyy') to datetime format 
